...
class EffectManager (context: Context) {
    ...
    fun makeVibration(duration: Long) {
        val vibrator = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.S) {
            ...
            contextRef.get()?.getSystemService(VIBRATOR_SERVICE) as Vibrator
        }
    ...
    fun updateAnimation(animationResourceId: Int, old: ImageView, new: ImageView) {
        val animation = AnimationUtils.loadAnimation(contextRef.get(), animationResourceId)
        old.clearAnimation()
        new.startAnimation(animation)
